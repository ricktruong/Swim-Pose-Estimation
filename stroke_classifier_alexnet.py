...
class StrokeAlexNet(nn.Module):
    def __init__(self, num_classes):
        super(StrokeAlexNet, self).__init__()
        # Input shape: (batch_size, 17, 2)
        
        # First conv block
        self.conv1 = nn.Sequential(
            nn.Conv1d(2, 64, kernel_size=11, stride=1, padding=5),
            nn.ReLU(),
            nn.BatchNorm1d(64),
...
def train_alexnet(X, y, num_classes, device='cuda' if torch.cuda.is_available() else 'cpu'):
    """Train an AlexNet classifier on the keypoint data."""
    # Split the data
    ...
    # Initialize model
    model = StrokeAlexNet(num_classes).to(device)
    ...
    torch.save(model.state_dict(), 'models/stroke_alexnet.pth')
    joblib.dump(scaler, 'models/stroke_alexnet_scaler.joblib')
    joblib.dump(label_encoder, 'models/stroke_alexnet_label_encoder.joblib')
    
    return model, scaler, label_encoder, test_accuracy

def predict_stroke_alexnet(frame_keypoints, model, scaler, label_encoder, device='cuda' if torch.cuda.is_available() else 'cpu'):
    """Predict stroke type for a single frame's keypoints using AlexNet."""
...
def main():
    ...
    model = StrokeAlexNet(num_classes=len(np.unique(y))).to(device)
    print_model_architecture(model)
    
    # Train the AlexNet classifier
    model, scaler, label_encoder, test_accuracy = train_alexnet(X, y, num_classes=len(np.unique(y)), device=device)
    ...
    prediction, probabilities = predict_stroke_alexnet(example_frame, model, scaler, label_encoder, device)
