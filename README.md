...
From breakthroughs in image processing techniques and computer vision, swim pose estimation now seems within reach. Via the Swim Analysis pipeline, the 4 components of which are listed below, we aim to classify the stroke of a swimmer of interest from individual frames in swimming videos. Further below are the steps required to run the Swim Analysis pipeline and conduct Pose Estimation.
